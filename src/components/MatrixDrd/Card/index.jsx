...
import drdImg from "../../../assets/hacker.png"

export default function CardDrd({ isDragging, id, img, description, title, idCell, correctCellId, className, showExpand = true }) {
    ...
    const onShowDetails = () => {
        mModalArticel({ description, title, img })
    }


    return (
        <>
            <DragPreviewImage connect={preview} src={drdImg} />
            ...
            <div ref={dragRef} style={{ opacity }} className={twMerge("w-full h-full bg-white shadow-sm rounded-sm  flex flex-row items-center ", className)} >
                <div className="group/item p-2 relative flex  items-center w-10/12">
    ...
    -translate-x-1/2  hidden  p-1 text-left">{title}</span>
                ...
                {showExpand && <div onClick={onShowDetails} className="flex-1 p-2 h-full  w-2/12 bg-[#6C757D] flex justify-center items-center cursor-pointer">
